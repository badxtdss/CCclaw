...
import { ensureCCCLAWContext, repairCCCLAWOnlyBootstrapFiles } from '../utils/openclaw-workspace';
...
const WINDOWS_APP_USER_MODEL_ID = 'app.ccclaw.desktop';
const isE2EMode = process.env.CCCLAW_E2E === '1';
const requestedUserDataDir = process.env.CCCLAW_USER_DATA_DIR?.trim();
...
// On Wayland this maps the running window to ccclaw.desktop (→ icon + app grouping);
// on X11 it supplements the StartupWMClass matching.
// Must be called before app.whenReady() / before any window is created.
if (process.platform === 'linux') {
  app.setDesktopName('ccclaw.desktop');
}
...
if (!gotElectronLock) {
  console.info('[CCCLAW] Another instance already holds the single-instance lock; exiting duplicate process');
  app.exit(0);
}
    ...
    if (!fileLock.acquired) {
      const ownerDescriptor = fileLock.ownerPid
        ...
        : fileLock.ownerFormat === 'unknown'
          ...
          : 'unknown owner';
      console.info(
        `[CCCLAW] Another instance already holds process lock (${fileLock.lockPath}, ${ownerDescriptor}); exiting duplicate process`,
      );
      app.exit(0);
    }
  } catch (error) {
    console.warn('[CCCLAW] Failed to acquire process instance file lock; continuing with Electron single-instance lock only', error);
  }
}
...
function createWindow(): BrowserWindow {
  ...
  const shouldSkipSetupForE2E = process.env.CCCLAW_E2E_SKIP_SETUP === '1';
...
async function initialize(): Promise<void> {
  ...
  logger.init();
  logger.info('=== CCCLAW Application Starting ===');
  logger.debug(
  ...
  session.defaultSession.webRequest.onHeadersReceived(
    { urls: ['http://127.0.0.1:18799/*', 'http://localhost:18799/*'] },
    (details, callback) => {
  ...
  // Repair any bootstrap files that only contain CCCLAW markers (no OpenClaw
  // template content). This fixes a race condition where ensureCCCLAWContext()
  // previously created the file before the gateway could seed the full template.
  if (!isE2EMode) {
    void repairCCCLAWOnlyBootstrapFiles().catch((error) => {
  ...
  gatewayManager.on('status', (status: { state: string }) => {
    ...
    if (status.state === 'running' && !isE2EMode) {
      void ensureCCCLAWContext().catch((error) => {
        logger.warn('Failed to re-merge CCCLAW context after gateway reconnect:', error);
      });
    }
  });
  ...
  // Merge CCCLAW context snippets into the workspace bootstrap files.
  // The gateway seeds workspace files asynchronously after its HTTP server
  // is ready, so ensureCCCLAWContext will retry until the target files appear.
  if (!isE2EMode) {
    void ensureCCCLAWContext().catch((error) => {
      logger.warn('Failed to merge CCCLAW context into workspace:', error);
    });
  }
...
if (gotTheLock) {
  ...
  app.on('second-instance', () => {
    logger.info('Second CCCLAW instance detected; redirecting to the existing window');

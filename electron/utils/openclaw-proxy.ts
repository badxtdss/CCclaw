...
interface SyncProxyOptions {
   ...
   * currently disabled in CCCLAW settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync CCCLAW global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because CCCLAW proxy is disabled and preserve mode is enabled');
      return;
    }

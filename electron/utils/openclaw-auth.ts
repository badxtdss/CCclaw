 ...
 * Used by CCCLAW to seed the provider store when it's empty but providers are
...
/**
 * Write the CCCLAW gateway token into ~/.openclaw/openclaw.json.
 */
export async function syncGatewayTokenToConfig(token: string): Promise<void> {
  return withConfigLock(async () => {
    ...
    // Packaged CCCLAW loads the renderer from file://, so the gateway must allow
 ...
 * which means conversations disappear after roughly one day.  CCCLAW sets
 ...
 * OpenClaw/CCCLAW versions or external tools.
 *
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  return withConfigLock(async () => {
    ...
    // When CCCLAW-managed moonshot provider exists, prefer centralized key
    ...
    // for CCCLAW to properly integrate with its updated tool system.
    ...
    // CCCLAW is a local desktop app where the user is the trusted operator.
    ...
    if (execConfig.security !== 'full' || execConfig.ask !== 'off') {
      ...
      toolsModified = true;
      console.log('[sanitize] Set tools.exec.security="full" and tools.exec.ask="off" to disable exec approvals for CCCLAW desktop');
    }
    ...
    if (typeof plugins === 'object' && !Array.isArray(plugins)) {
      ...
      // id="openclaw-qqbot" in its manifest, but older CCCLAW versions
    ...
    // but CCCLAW historically stored them only under `channels.<type>.accounts.default`.

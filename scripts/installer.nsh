; CCCLAW Custom NSIS Installer/Uninstaller Script
;
...
!macro customCheckAppRunning
  ...
  DetailPrint "Extracting CCCLAW runtime files. This can take a few minutes on slower disks or while antivirus scanning is active."
  ...
  ${if} $R0 == 0
    ...
    # This covers CCCLAW.exe (multiple Electron processes), openclaw-gateway.exe,
  ...
  ; Even if CCCLAW.exe was not detected as running, orphan child processes
  ...
  ; PowerShell path-based kill may miss processes if the old CCCLAW was installed
  ...
  ; "Can't modify CCCLAW's files" retry loop -> "CCCLAW 无法关闭" dialog.
  ;
  ...
  ; files (antivirus, indexing) causing a blocking "CCCLAW 无法关闭" dialog.
...
; "CCCLAW 无法关闭" retry dialog when the old uninstaller fails.
;
...
!macro customInstall
  ...
  ; Wait 60s before starting deletion to avoid I/O contention with CCCLAW's
...
!macro customUnInstall
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "Do you want to remove CCCLAW application data?$\r$\n$\r$\nThis will delete:$\r$\n  • AppData\Local\ccclaw (local app data)$\r$\n  • AppData\Roaming\ccclaw (roaming app data)$\r$\n$\r$\nYour .openclaw folder (configuration & skills) will be preserved.$\r$\nSelect 'No' to keep all data for future reinstallation." \
    /SD IDNO IDYES _cu_removeData IDNO _cu_skipRemove

  _cu_removeData:
    ; Kill any lingering CCCLAW processes (and their child process trees) to
    ...
    RMDir /r "$LOCALAPPDATA\ccclaw"
    RMDir /r "$APPDATA\ccclaw"

    ; --- Retry: if directories still exist (locked files), wait and try again ---

    ; Check AppData\Local\ccclaw
    IfFileExists "$LOCALAPPDATA\ccclaw\*.*" 0 _cu_localDone
      Sleep 3000
      RMDir /r "$LOCALAPPDATA\ccclaw"
      IfFileExists "$LOCALAPPDATA\ccclaw\*.*" 0 _cu_localDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$LOCALAPPDATA\ccclaw"'
        Pop $0
        Pop $1
    _cu_localDone:

    ; Check AppData\Roaming\ccclaw
    IfFileExists "$APPDATA\ccclaw\*.*" 0 _cu_roamingDone
      Sleep 3000
      RMDir /r "$APPDATA\ccclaw"
      IfFileExists "$APPDATA\ccclaw\*.*" 0 _cu_roamingDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$APPDATA\ccclaw"'
        Pop $0
        Pop $1
    _cu_roamingDone:
    ...
    StrCpy $R3 ""
    IfFileExists "$LOCALAPPDATA\ccclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $LOCALAPPDATA\ccclaw"
    IfFileExists "$APPDATA\ccclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $APPDATA\ccclaw"
  ...
  _cu_enumLoop:
    ...
    RMDir /r "$R3\AppData\Local\ccclaw"
    RMDir /r "$R3\AppData\Roaming\ccclaw"
